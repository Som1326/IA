...
def busqueda_A_estrella(ciudad):
    ...
    heapq.heappush(cola_prioridad, (0 + heuristica(inicio, pasajero), inicio, False, [], 0))  # (h(n), posición, tiene pasajero, ruta)
    visitados = set()
            ...
            if es_valida((nuevo_x, nuevo_y), ciudad):
                ...
                if (nueva_pos, nuevo_tiene_pasajero) not in visitados:
                    ...
                    if nuevo_tiene_pasajero:
                        ...
                        heapq.heappush(cola_prioridad, (nuevo_costo + heuristica(nueva_pos, destino), nueva_pos, nuevo_tiene_pasajero, nueva_ruta, nuevo_costo))
                    else:
                        # Añadir a la cola de prioridad con el valor de la heurística al pasajero
                        heapq.heappush(cola_prioridad, (nuevo_costo + heuristica(nueva_pos, pasajero), nueva_pos, nuevo_tiene_pasajero, nueva_ruta, nuevo_costo))
